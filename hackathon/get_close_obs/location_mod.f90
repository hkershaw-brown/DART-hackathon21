...
if(num_close /= cnum_close) then
   ...
   call error_handler(E_MSG, 'get_close', msgstring, source, &
...
do i = 1, my_num_obs
  if (abs(cdist(i) - sorted_dist(i)) > tolerance) then 
    print*, 'not equal', cdist(i), sorted_dist(i)
  else
    print*, 'equal', cdist(i), sorted_dist(i) 
  endif
enddo
